<template>
	<el-main class="bg table_wrap">
		...
		<el-table border :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" stripe>
			...
			<el-table-column fixed prop="name" label="名称" min-width="180"></el-table-column>
			...
			<el-table-column fixed="right" label="操作" width="180">
				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--primary" v-if="$check_action('/evaluation/table','set')"
						:to="'./view?' + field + '=' + scope.row[field]" size="small">
            详情
					</router-link>
					<router-link class="el-button el-button--small is-plain el-button--primary" v-if="$check_exam('/evaluation/table','setting')"
								 :to="'/evaluation_question_table/table?' + field + '=' + scope.row[field]" size="small">
						题库
					</router-link>
				
					<router-link class="el-button el-button--small is-plain el-button--primary"  :data-num='answered_list[scope.row[field]]' style="margin-left: 0;margin-top: 10px" v-if="($check_exam('/evaluation/table','answers')) && answered_list[scope.row[field]]"
								 :to="'/evaluation_answer_view/view?' + field + '=' + scope.row[field]" size="small">
						<span>查看</span>
					</router-link>
					<router-link class="el-button el-button--small is-plain el-button--primary" style="margin-left: 0;margin-top: 10px" v-else-if="($check_exam('/evaluation/table','answers')) && scope.row.status==='启用'"
								 :to="'/evaluation_answer_view/view?' + field + '=' + scope.row[field]" size="small">
						<span>答题</span>
					</router-link>
					
					<router-link class="el-button el-button--small is-plain el-button--primary" style="margin-top: 10px" v-if="$check_exam('/evaluation/table','give_score')"
