<template>
	<div class="diy_home diy_list diy_rainfall_data" id="diy_rainfall_data_list">
		<!-- 列表 -->
		<div class="diy_view_list list list-x">
					<router-link class="diy_card goods diy_list_box_wrap" v-for="(o, i) in list" :key="i"
				:to="'/rainfall_data/details?rainfall_data_id=' + o['rainfall_data_id']">
										<!-- 图片 -->
				<div class="diy_list_img_box" v-if="imgList.length" >
					<div class="diy_row" v-for="(item,index) in imgList" :key="item+index" v-show="$check_field('get',item.name,'/rainfall_data/details') && +item.is_img_list">
				...
				<div class="diy_list_item_box">
					<div class="diy_list_item_content" v-for="(item,index) in showItemList" :key="item+index">
						<div class="diy_row" :class="{[item.name]:true}" v-if="$check_field('get',item.name,'/rainfall_data/details') && +item.is_img_list">
							<div class="diy_title">
								<span>{{item.title}}:</span>
							</div>
		...
		<!-- 轮播 -->
		 ...
		 <div class="carousel ins_s">
                <div class="slider" ref="slider">
                    ...
                    <router-link :to="'/rainfall_data/details?rainfall_data_id=' + o['rainfall_data_id']" class="lis_cont">
						                        <div class="diy_list_img_box" v-if="imgList.length" >
    					        <div class="diy_row" v-for="(item, index) in imgList" :key="item + index" v-show="$check_field('get',item.name,'/rainfall_data/details') && +item.is_img_list">
    						<div class="diy_title diy_list_img_title">
...
<script>
	export default {
			...
			return {
				...
				totalItems: 3,
						...
						itemList: [
								...
								{
									title: "雨量最大值",
									name: "maximum_rainfall",
									type: "数字",
									is_img_list: "0"
								},
								{
									title: "雨量最小值",
									name: "minimum_rainfall",
									type: "数字",
									is_img_list: "0"
								},
								{
									title: "雨量标准值",
									name: "rainfall_standard_value",
									type: "数字",
									is_img_list: "0"
								},
								{
									title: "雨量预警值",
									name: "rainfall_warning_value",
									type: "数字",
									is_img_list: "0"
								},
						],
		...
		methods: {
			...
			to_detail(v){
				this.$router.push("/rainfall_data/details?rainfall_data_id="+v.rainfall_data_id)
			},
