			...
			<div class="container">
				...
				<div class="row row_det">
						<div v-if="this.$store.state.user.user_id" class="btns_add col-12 col-sm-6">
							<div v-if="$check_action('/weather_information/details','set')">
								<!-- 点赞按钮 -->
								<div
										v-if="state_praise"
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/weather_information/set?weather_information_id=' + _this.obj["weather_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.region_name
																			query.img = this.obj.weather_pictures
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var weather_information_id = this.obj.weather_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "weather_information",
          source_field: "weather_information_id",
          source_id: weather_information_id
        }, (res) => {
